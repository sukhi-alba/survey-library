...
export function TimePickerRenderer({ question }: TimePickerProps) {
  ...
  const onChange = (event: DateTimePickerChangeEvent, selectedDate?: Date) => {
    ...
    if (selectedDate) {
      question.value = formatTime(selectedDate);
    }
  };

  const displayText = value ? value : (question.placeholder || "Select Time");
